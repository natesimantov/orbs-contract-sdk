...
ln -f -s ../orbs-network-go/gamma-cli gamma-cli
ln -f -s ../orbs-network-go/gamma-server gamma-server

# Create global symlinks
sudo ln -f -s $GOPATH/src/github.com/orbs-network/orbs-network-go/gamma-cli /usr/local/bin/gamma-cli
sudo ln -f -s $GOPATH/src/github.com/orbs-network/orbs-network-go/gamma-server /usr/local/bin/gamma-server
